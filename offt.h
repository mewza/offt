...
public:
    ...
    OFFT() : _initialized(false)
    {
    }
    
    void real_fft(const T* in, ComplexT* out, int N, bool do_scale = false)
    {
        if (!_initialized)
            init(N);
        
        memcpy(out, in, N * sizeof(T));
        rdft(N, 1, (T*)out, _ip.data(), _w.data());
        ...
        if (do_scale) {
            ScaleBuffer(out, 2./(T1)N, N);
        }
    }
    
    void real_ifft(const ComplexT* in, T* out, int N, bool do_scale = false)
    {
        if (!_initialized)
            init(N);
        memcpy(_buffer.data(), in, (N+1)*sizeof(ComplexT));
        _buffer[1] = in[N].re;
        rdft(N, -1, _buffer.data(), _ip.data(), _w.data());
        if (do_scale) {
            ScaleBuffer(out, _buffer.data(), 2./(T1)N, N);
        } else {
            memcpy(out, _buffer.data(), N * sizeof(T));
        }
    }
...
protected:
    ...
    void init(int N)
    {
        ...
        _ip.resize(2 + (int)std::sqrt(N));
        _w.resize(N / 2);
        _buffer.resize((N+1)*sizeof(ComplexT)/sizeof(T));
    ...
    inline void TransferBuffer(T* dest, const T* src, int len)
    {
        for (int i=0; i<len; i++) {
            dest[i] = src[i];
        }
    ...
    inline void ScaleBuffer(T* dest, const T* src, const T1 factor, int len)
    {
        for (int i=0; i<len; i++) {
            dest[i] = src[i] * factor;
        }
    ...
    inline void ScaleBuffer(ComplexT* dest, const T1 factor, int len)
    {
        for (int i=0; i<len; i++) {
            dest[i] *= factor;
        }
    ...
    void makewt(int nw, int *ip, T *w)
    {
        ...
        if (nw > 2) {
            nwh = nw >> 1;
            delta = F_ATAN(1.0) / nwh;
            w[0] = 1;
            w[1] = 0;
            w[nwh] = F_COS(delta * nwh);
            w[nwh + 1] = w[nwh];
            if (nwh > 2) {
                for (j = 2; j < nwh; j += 2) {
                    x = F_COS(delta * j);
    ...
    void makect(int nc, int *ip, T *c)
    {
        ...
        if (nc > 1) {
            nch = nc >> 1;
            delta = F_ATAN(1.0) / nch;
            c[0] = F_COS(delta * nch);
            c[nch] = 0.5 * c[0];
            for (j = 1; j < nch; j++) {
                c[j] = 0.5 * F_COS(delta * j);
                c[nc - j] = 0.5 * F_SIN(delta * j);
            }
        }
    }
